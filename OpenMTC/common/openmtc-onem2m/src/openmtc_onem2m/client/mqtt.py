...
class OneM2MMQTTClient(OneM2MClient):
    ...
    def __init__(self, m2m_ep, _=None, client_id=None, handle_request_func=None, subscribe_sys_topics=False,
                 ca_certs=None, cert_file=None, key_file=None, insecure=False):
        """
        :param str m2m_ep:
        :param bool _:
        ...
        if client_id is None:
            import random
            import string
            # client_id = ''.join(random.sample(string.letters, 16)) # string.letters is for Python2 only
            client_id = ''.join(random.sample(string.ascii_letters, 16))
        else:
    ...
    def send_onem2m_request(self, request):
        ...
        self._publish_message(
            ...
            self._build_topic(originator=client_id, receiver=target_id) + '/json',
        )
